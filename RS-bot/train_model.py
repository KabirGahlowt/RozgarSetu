...
def train_model():
    ...
    for _, row in workers_df.iterrows():
        workers.append(data_loader.get_worker(int(row['worker_id'])))
    
    jobs = []
    for _, row in jobs_df.iterrows():
        jobs.append(data_loader.get_job(int(row['job_id'])))
    
    print("Computing CBF scores...")
    cbf = ContentBasedFilter()
    cbf_scores = []
    for idx, row in interactions_df.iterrows():
        worker = data_loader.get_worker(int(row['worker_id']))
        job = data_loader.get_job(int(row['job_id']))
